...
#    - generally, these are the envdata extracted from ClimateNA GIS data, and then clipped to the range map
...
# predfile - path to RDS of object saved from interpolating gradient forest model across landscape for the same time period as the data that went into training (created in gradient_training.R)
...
get_offset <- function(outfile){
    ...
    ptm <- proc.time()
    projOut <- predict(gfOut, valid_data[, envs])
    print(proc.time() - ptm)
...
range_cells <- cellFromXY(mask, valid_data[ ,c('lon','lat')])
